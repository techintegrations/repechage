...
document.querySelector('.add-to-cart-F-B').addEventListener('click', async () => {
  // Add the products to the cart first
  await addProductsToCart(productVariantIds);

  // After products are added, call the buildCart function to update the cart
  if (typeof theme.cart.buildCart === 'function') {
    theme.cart.buildCart(); // This will trigger the jQuery/cart update
  } else {
    console.error('buildCart function is not defined.');
  }

  // Open the cart drawer after the cart is updated
